...
На сегодняшний день, считается обязательной подпись электронных писем
с помощью [DKIM](http://www.dkim.org/). Это делается для того, чтобы почтовый сервер
...
Даже пройдя верификацию по `dkim` и `spf` записям, всеравно уходят в нежелательную почту.
