...
Для большей наглядности, приведем пример неправильной обрезки изображения из-за
кеширования, а так же различного поведения изображения с `exif` ориентацией
...
Возьмем портретно ориентированное изображение, и попробуем его дважды обрезать
на одном и том же превью изображении.

![lenna](/media/wrong-exif/lenna.jpg
Исходное изображение.){.center}

Модератор обрезает изображение, затем после обновления страницы повторно
загружается тоже-самое превью. Модератор обрезает изображение второй раз,
думая при этом, что изменения по какой либо причине не сохранились.

![lenna](/media/wrong-exif/lenna_cut.png
Превью обрезки изображения.){.center}

Но, на самом деле, изображение в первый раз обрезалось корректно.

![lenna](/media/wrong-exif/lenna_cache_crop_1.jpg
Правильно обрезанное изображение.){.center}

Получив повторные координаты для обрезки изображения, происходит следующее.

![lenna](/media/wrong-exif/lenna_cache_crop_2.jpg
Дважды обрезанное изображение.){.center}

И так далее, при каждой последующей обрезки исходное изображение будет
продолжать кромсаться. Поэтому случайный `GET` параметр позволяет бороться с
кешированием и загружать необходимое превью для правильной обработки
изображения.
