...
Summary:
    ...
    использовать не приходиться. Из асинхронных задач сталкиваюсь только с `Celery`,
...
- Создать комнату или подключиться к уже существующей.
...
Текущая версия `AioHTTP 2.0.6`, однако оказалось, что с ней несовместима
...
работает это немного по-другому и с непривычки вызывает недоумение.
...
Чтобы получить текущюю сессию, нужно дождаться ответа из корутины `get_session`.
...
Для хранения текущих соединений и рассылки бродкаст сообщений,
...
По умолчанию `Nginx` не проксирует заголовки `Upgrade` и `Connection`,
...
Мне нравится подход [Unittest](http://aiohttp.readthedocs.io/en/stable/testing.html#unittest).
...
Cоздание тестовой базы данных через `PeeWee Async` оказалось не очень удобно,
поэтому тесты будут весьма поверхностные,
только чтобы обозначить общий подход к тестированию приложения на `AioHTTP`.
...
иначе всё начнет превращаться в сплошную рутину-корутины.
